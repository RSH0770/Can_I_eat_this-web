...
import {
  MOCK_ALLERGIES,
  MOCK_DISEASES,
} from "../../../constants/mockUserProfile";
...
export function OrderCard() {
  ...
  // 화면/캔버스가 실제로 그릴 데이터
  // cardId 모드면 서버가 다듬어 준 CardResponse를, 아니면 로컬 계산값을 씀
  // cardId가 있는데 아직 loading/error면 null(이때는 카드 박스 자체를 그리지 않음)
  ...
  return (
    ...
    >
      ...
      <div className="no-scrollbar min-h-0 flex-1 overflow-x-hidden overflow-y-auto">
        {cardId != null && cardState.status === "loading" && (
          <p className="text-[0.9375rem]">카드를 불러오는 중...</p>
        )}

        {cardId != null && cardState.status === "error" && (
          <div>
                  ...
                  <div className="flex flex-col gap-[12px]">
                    {cardData.requestLines.map((r, i) => (
                      ...
                      >
                        ...
                        <span className="text-[1.125rem] leading-[1.55]">
                          {r}
                        </span>
                      </div>
                    ))}
